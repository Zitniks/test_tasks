"""Flight filtering, comparison and selection utilities."""

from internal.models.flight import FlightItinerary


def filter_flights_by_route(
    itineraries: list[FlightItinerary], source: str, destination: str
) -> list[FlightItinerary]:
    return [it for it in itineraries if it.get_source() == source and it.get_destination() == destination]


def find_cheapest(itineraries: list[FlightItinerary]) -> FlightItinerary | None:
    if not itineraries:
        return None
    return min(itineraries, key=lambda x: x.get_total_price())


def find_most_expensive(itineraries: list[FlightItinerary]) -> FlightItinerary | None:
    if not itineraries:
        return None
    return max(itineraries, key=lambda x: x.get_total_price())


def find_fastest(itineraries: list[FlightItinerary]) -> FlightItinerary | None:
    if not itineraries:
        return None
    return min(itineraries, key=lambda x: x.get_total_duration_minutes())


def find_slowest(itineraries: list[FlightItinerary]) -> FlightItinerary | None:
    if not itineraries:
        return None
    return max(itineraries, key=lambda x: x.get_total_duration_minutes())


def find_optimal(itineraries: list[FlightItinerary]) -> FlightItinerary | None:
    if not itineraries:
        return None
...
def compare_itineraries(
    itineraries1: list[FlightItinerary], itineraries2: list[FlightItinerary]
) -> dict:
    """Compare two itinerary lists; return only_in_first, only_in_second, price_changes, totals."""

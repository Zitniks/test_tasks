"""FastAPI application: Aviasales Flight Search API."""
...
def get_itineraries(request: Request) -> tuple[list[FlightItinerary], list[FlightItinerary]]:
    """Dependency: return loaded itineraries from app.state."""
...
async def lifespan(app: FastAPI):
    ...
    try:
        app.state.itineraries_1 = parse_xml_file(str(file1_path))
        app.state.itineraries_2 = parse_xml_file(str(file2_path))
    except Exception as e:
        raise RuntimeError(f'Failed to load flight data: {e}') from e
    yield
...
def get_flights_dxb_bkk(
    itineraries: tuple[list[FlightItinerary], list[FlightItinerary]] = Depends(get_itineraries),
):
    _, itineraries_2 = itineraries
...
@app.get(
    ...
    responses={
        status.HTTP_200_OK: {'description': 'Самый дешёвый вариант перелёта найден'},
        status.HTTP_404_NOT_FOUND: {'description': 'Перелёты не найдены'},
...
@app.get(
    ...
    responses={
        status.HTTP_200_OK: {'description': 'Самый дорогой вариант перелёта найден'},
        status.HTTP_404_NOT_FOUND: {'description': 'Перелёты не найдены'},
        status.HTTP_500_INTERNAL_SERVER_ERROR: {'description': 'Internal server error'},
    },
)
...
@app.get(
    ...
    responses={
        status.HTTP_200_OK: {'description': 'Самый быстрый вариант перелёта найден'},
        status.HTTP_404_NOT_FOUND: {'description': 'Перелёты не найдены'},
        status.HTTP_500_INTERNAL_SERVER_ERROR: {'description': 'Internal server error'},
    },
)
...
@app.get(
    ...
    responses={
        status.HTTP_200_OK: {'description': 'Самый долгий вариант перелёта найден'},
        status.HTTP_404_NOT_FOUND: {'description': 'Перелёты не найдены'},
        status.HTTP_500_INTERNAL_SERVER_ERROR: {'description': 'Internal server error'},
    },
)
...
@app.get(
    ...
    responses={
        status.HTTP_200_OK: {'description': 'Оптимальный вариант перелёта найден'},
        status.HTTP_404_NOT_FOUND: {'description': 'Перелёты не найдены'},
        status.HTTP_500_INTERNAL_SERVER_ERROR: {'description': 'Internal server error'},
    },
)
...
def compare_flights(
    ...
    itineraries_1, itineraries_2 = itineraries
